. ".shared\functions\write.ps1"
. ".shared\functions\env.ps1"
. ".shared\functions\dotenv.ps1"
. ".shared\functions\command.ps1"
. ".shared\functions\modules.ps1"
. ".shared\functions\io.ps1"
. ".shared\functions\common.ps1"
. ".shared\functions\clipboard.ps1"
# ----------------------------------------------------------------------------------------------------------------------
. ".shared\functions\ssh.ps1"               # index file for ssh functions
# ----------------------------------------------------------------------------------------------------------------------
. ".shared\functions\git-test.ps1"          # git repository testing functions
. ".shared\functions\git-config.ps1"        # git configuration functions
# ----------------------------------------------------------------------------------------------------------------------
. ".shared\functions\dotnet-install.ps1"    # dotnet installation functions
. ".shared\functions\dotnet-project.ps1"    # dotnet project functions
# ----------------------------------------------------------------------------------------------------------------------
. ".shared\functions\choco-install.ps1"     # chocolatey installation functions
. ".shared\functions\cli-install.ps1"       # cli installation functions

# ----------------------------------------------------------------------------------------------------------------------